...
#define AUTH_INFO	"9OhwBvAltkCoTW1nUoqDZEyMpys="

#define DEVID		"512405403"
...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"uv_value\":%0.3f", uv_value);
	strcat(buf, text);
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBACK:														//发送Publish消息，平台回复的Ack
			...
			if(MQTT_UnPacketPublishAck(cmd) == 0)
				UsartPrintf(USART_DEBUG, "Tips:	MQTT Publish Send OK,succed!!!\r\n");
